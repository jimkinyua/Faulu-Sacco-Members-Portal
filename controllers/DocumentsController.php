...
use app\models\PayslipAttachements;
...
class DocumentsController extends \yii\web\Controller
{
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'only' => [
                    'getloans', 'create', 'index', 'read',
                    'delete-attachement', 'update',
                ],
                'rules' => [
                    [
                        'actions' => [
    ...
    {
        $model = new PayslipAttachements();
        $LoanModel = $this->GetLoanDetails($Key);
        $model->DocNum = $LoanModel->Application_No;
        $model->MemberNo = $LoanModel->Member_No;
        $model->DocumentsAttached = false;
        $attachements = $model->getLoanAttachments();

        //Check if is SelfEmployed
        if (Yii::$app->user->identity->{'Employer Code'} == env('SelfEmployedCode')) {
            $model->isSelfEmployed = 1;
        } else {
            $model->isSelfEmployed = 0;
        }
        ...
        if ($attachements === false) {
            ...
            foreach ($attachements as $attachement) {
                if ($attachement->Document_No == 'LETTER') {
                    $model->LetterAttached = true;
                }
                if ($attachement->Document_No == 'CERTIFIEDBANKSTATEMENTS') {
                    $model->CertifiedBankStatementsAttached = true;
                }
                if ($attachement->Document_No == 'PAYSLIPONE') {
                    $model->PayslipOneAttached = true;
                }
                if ($attachement->Document_No == 'PAYSLIPTWO') {
                    $model->PayslipTwoattached = true;
                }
            }
            // if(){

            // }
            $model->DocumentsAttached = true;
        }
        // echo '<pre>';
        // print_r($model);
        // exit;
        ...
        if (Yii::$app->request->isAjax) {
            $model->Type = 'Loan Application Documents';
            $model->DocNum = $LoanModel->Application_No;
            $model->PayslipOne = UploadedFile::getInstance($model, 'PayslipOne');
            $model->PayslipTwo = UploadedFile::getInstance($model, 'PayslipTwo');
            $model->Letter = UploadedFile::getInstance($model, 'Letter');
            $model->CertifiedBankStatements = UploadedFile::getInstance($model, 'CertifiedBankStatements');
            if ($model->validate()) {
                $uploadResult = $model->upload();
                if ($uploadResult === true) {
                    return $this->redirect(['/guarantors', 'Key' => $LoanModel->Key]);
                }
                return $this->asJson(['error' => $uploadResult]);
            }
            $result = [];
        ...
        return $this->render('index', [
            'LoanModel' => $LoanModel,
            'model' => $model,
            'RequiredAttachements' => $this->getRequiredLoanAttachements(),
    ...
    public function getRequiredLoanAttachements()
    {
        $res = [
            ['Id' => 1, 'Description' => 'Payslip'],
            ['Id' => 2, 'Description' => 'Letter'],
            ['Id' => 3, 'Description' => 'Certified Bank Statements'],
            ['Id' => 4, 'Description' => 'Funds Transfer Form'],
        ];
        return $res;
    }
