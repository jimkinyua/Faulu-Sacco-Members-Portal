...
use app\models\MpesaAccountDeposit;


class AccountDepositController extends \yii\web\Controller
{
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'only' => ['getloans', 'create', 'index', 'update'],
                'rules' => [
                    [
                        'actions' => ['getloans', 'index', 'create', 'update'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
    ...
    public function actionIndex($Account)
    {
        ...
        if (Yii::$app->request->post() && $this->loadpost(Yii::$app->request->post()['MpesaAccountDeposit'], $model)) {

            $PhoneNo =  ltrim($model->STKpushNo, '+254');
            // $model->LoanNo = $result->Application_No;
            //Do STK Manenos
            $StkResult =  Yii::$app->MpesaIntergration->createPushSTKNotificationForAccountDeposit($PhoneNo, $model);
