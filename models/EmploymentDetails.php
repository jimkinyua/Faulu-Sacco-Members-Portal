...
class EmploymentDetails extends Model
{
    ...
    {
        return [
            [['Designation', 'Employer_Code',], 'required'],

            [['Occupation',], 'required', 'when' => function ($model) {
                return $model->Employer_Code == env('SelfEmployedCode');
            }, 'whenClient' => "function (attribute, value) {
                return $('#employmentdetails-employer_code').val() == 227;
            }"],

            [['Payroll_No',], 'string', 'max' => 8, 'when' => function ($model) {
                return $model->Employer_Code != '227';
            }, 'whenClient' => "function (attribute, value) {
                return $('#employmentdetails-employer_code').val() != 227;
            }"],
            ...
            [['Payroll_No',], 'required', 'when' => function ($model) {
                return $model->Employer_Code != '227';
            }, 'whenClient' => "function (attribute, value) {
                return $('#employmentdetails-employer_code').val() != 227;
            }"],
