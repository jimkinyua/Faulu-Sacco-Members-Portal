...
class NomineeAttachements extends Model
{
        ...
        if($model->validate()){
            ...
            if($this->docFile->saveAs($imagePath)){
                ...
                if($AlreadyExists){ //Do An Update
                    ...
                    $UploadToEDMSResult = Yii::$app->Mfiles->UpdateNomineeDocumentOnEDMS($model);
                ...
                $UploadToEDMSResult = Yii::$app->Mfiles->UploadNomineeDocumentToEDMS($model);
                if($UploadToEDMSResult['Uploaded']== false){
                    Yii::$app->session->setFlash('error',$UploadToEDMSResult['Message']);
                    return false;
                }else{
    ...
    public function CreateNomineeOnEDMS($NomineeModel){
        $CreateNomineeResult = Yii::$app->Mfiles->CreateNominee($NomineeModel);
        if($CreateNomineeResult['Exists']== true || $CreateNomineeResult['Created']== true){
            return true;
        }
        return false;
    ...
    public function SaveDocumentToNav($model, $MfilesUrl){
        $service = Yii::$app->params['ServiceName']['NomineeAttachments'];
