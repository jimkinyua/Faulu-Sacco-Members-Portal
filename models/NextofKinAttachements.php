...
class NextofKinAttachements extends Model
{
    ...
    public function upload($docId = false)
    {
        ...
        if ($model->validate()) {
            ...
            if ($this->docFile->saveAs($imagePath)) {
                ...
                // }else{
                //Post to Nav
                return true; //$this->SaveDocumentToNav($model, $UploadToEDMSResult['DocumentUrl']);
                // }
            }
        } else {
            // return false;
    ...
    public function CreateKinOnEDMS($KinModel)
    {
        $CreateApplicantResult = Yii::$app->Mfiles->CreateKin($KinModel);
