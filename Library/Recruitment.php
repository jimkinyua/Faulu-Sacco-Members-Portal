...
class Recruitment extends Component
{
    ...
    public function UserhasImageAndSignature()
    {

        if ($this->getMemberImage() === false || $this->getMemberSignature() === false) {
            return false;
        }
        return true;
    }
    ...
    public function CheckIfMemberHasPriorPendingDocument($documentType)
    {
        $service = Yii::$app->params['ServiceName']['PortalIntegrations'];
        ...
        $response = Yii::$app->navhelper->PortalReports($service, $data, 'CheckMemberPendingDocument');
        // echo '<pre>';
        // print_r($response);
        // exit;
        ...
        if (is_array($response)) {
            ...
            if (isset($response['return_value']) && $response['return_value'] == '00') { //Has No Document
                return false;
            } else {
                return true;// Has Document
            }
        }
        return false;
    }

    public function LogActionHAsBeenAccessed($data, $action)
    {
        $LogModel = new LogActionModel();
        return $LogModel->log($data, $action);
    }
    ...
    public function getMemberName($MemberNo)
    {
        $member = user::findIdentity($MemberNo);
        if ($member) {
            return $member->{'Full Name'};
        }
    }

    public function getLoanNoFromGuarantorShipRequest($DocNo)
    {
        ...
        return $guarantors[0]->Loan_No;
    }

    public function getLoanDetails($DocNo)
    {
        $service = Yii::$app->params['ServiceName']['PostedLoanCard'];
        $filter = [
            'Application_No' => $DocNo,
        ];
        $guarantors = Yii::$app->navhelper->getData($service, $filter);
        if (is_object($guarantors)) {
            return [];
