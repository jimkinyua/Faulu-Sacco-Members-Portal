                    ...
                    <?php if(!is_object($GuarantorSubstituitionRequests)): ?>
                        <?php foreach($GuarantorSubstituitionRequests as $GuarantorSubstituitionRequest): ?> 
                            <tr>
                                ...
                                <td><span class="font-weight-normal"><?= @number_format($GuarantorSubstituitionRequest->Guarantor_Value) ?></span></td>
                                <td><span class="font-weight-normal"><?= @$GuarantorSubstituitionRequest->ApplicantName ?></span></td>
                                <td><span class="font-weight-normal"><?= @number_format($GuarantorSubstituitionRequest->Loan_Principal) ?></span></td>
                                <?php
                                    $acceptLink = Html::a('Accept',Url::to(['guarantors/accept','Key'=> urlencode($GuarantorSubstituitionRequest->Key) ]) ,['class'=>'update btn btn-info btn-md']);
                                    $rejectlink = Html::a('Reject',Url::to(['guarantors/reject','Key'=> urlencode($GuarantorSubstituitionRequest->Key) ]), ['class'=>'update btn btn-danger btn-md']);
                                ?>
                                    <td><span class="font-weight-normal"><?= $acceptLink .' '. $rejectlink ?></span></td>                                                                            
