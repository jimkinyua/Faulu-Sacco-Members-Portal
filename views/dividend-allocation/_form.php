...
<?php $form = ActiveForm::begin(['options' => ['enctype' => 'multipart/form-data']]); ?>
    <div class="card-body">
            ...
            <div class="row">
                ...
                <div class=" row col-md-12">
                        ...
                        <?= $form->field($model, 'Net_Amount')->textInput(['readonly' => 'true', 'value'=>number_format($model->Net_Amount)]) ?>
                                ...
                                ?>
                                <?= $form->field($model, 'MPESA_No')->widget(PhoneInput::className(), [
                                    'jsOptions' => [
                                        'allowExtensions' => false,
                                        'onlyCountries' => ['ke'],
                                ]]) ?>

                                <?= $form->field($model, 'Allocation_Account')->dropDownList([
                                    ],['prompt' => '-- Select Option --', ]) 
                                ?>
            ...
            <?= Html::submitButton('Submit Details', [
        ...
        $('#payment-guideline').hide();
            ...
            if(Source == 'MPESA_Payment' || Source == 'Bank_Payment'){
                ...
                if(Source != 'MPESA_Payment'){ //Bank
                    ...
                    $('#dividendallocationheader-bank_code').append($('<option id="itemId"></option>').attr('value', '').text('-- Select Bank --')); //append Here
                ...
                if(Source == 'Loan_Payment'){
                    ...
                    $('#dividendallocationheader-allocation_account').append($('<option id="itemId"></option>').attr('value', '').text('-- Select Loan --')); //append Here
                        $.each(response, function (key, entry) {
                            $('#dividendallocationheader-allocation_account').append($('<option id="itemId'+ entry.Code+'"></option>').attr('value', entry.Code).text(entry.Name)); //append Here
                        })
                    });
